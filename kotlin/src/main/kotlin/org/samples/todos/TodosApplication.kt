...
class TodosApplication {
    ...
    constructor() {
        ...
        val taskRepository = TaskRepository(TASK_FILE_PATH, objectMapper)
        ...
        while (true) {
            ...
            if (StringUtils.isNumeric(choice)) {
                when (choice.toInt()) {
                    ...
                    10 -> exit()
                    else -> System.out.println("Invalid choice. Select again.")
                }
            } else {
                println("Validation error: choice must be number like (1-10).")
            }
    ...
    private fun displayAllTasks() {
        ...
        for (taskGroup in taskGroups) {
            ...
            println("-----")
            taskGroup.tasks.forEach { println(it) }
        }
    }
    ...
    private fun createTask() {
        ...
        if (!listOf("LOW", "MEDIUM", "HIGH").contains(priority.uppercase())) {
    ...
    private fun updateTask() {
        ...
        val task = Task(
            ...
            newTitle,
            newDescription,
            Priority.valueOf(newPriority.uppercase()),
            false,
            Date()
        )
    ...
    private fun getTasksByGroupName() {
        ...
        val tasks = taskManager.getBy(groupName)
        println(tasks)
    }
    ...
    private fun getTasksByGroupNameAndPriority() {
        ...
        val tasks = taskManager.getBy(groupName, Priority.valueOf(priority.uppercase()))
        println(tasks)
    }
    ...
    private fun getTasksByGroupNameAndDone() {
        ...
        val tasks = taskManager.getBy(groupName, done.toBoolean())
        println(tasks)
    }
    ...
    private fun getTasksByGroupNameAndDate() {
        ...
        val tasks = taskManager.getBy(groupName, parsedDate)
        println(tasks)
    }
